...
public class MyExpandTextView extends TextView implements View.OnClickListener
{
    ...
    private void init(AttributeSet attrs)
    {
        ...
        mMaxCollapsedLines = typedArray.getInt(
                                 R.styleable.ExpandableTextView_maxCollapsedLines, MAX_COLLAPSED_LINES);
        mAnimationDuration = typedArray.getInt(
                                 R.styleable.ExpandableTextView_animDuration, DEFAULT_ANIM_DURATION);
        mExpandDrawable = typedArray.getDrawable(
                              R.styleable.ExpandableTextView_expandDrawable);
        mCollapseDrawable = typedArray.getDrawable(
                                R.styleable.ExpandableTextView_collapseDrawable);

        mExpandDrawablePadding = 3 * 3;

        if (mExpandDrawable == null) {
            mExpandDrawable = mContext.getResources().getDrawable(
                                  R.drawable.ic_expand_small_holo_light);
        }

        if (mCollapseDrawable == null) {
            mCollapseDrawable = mContext.getResources().getDrawable(
        ...
        mDrawableHeight = mExpandDrawable.getMinimumHeight() + mExpandDrawablePadding *
                          2;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        mExpandTextHeight = getLayout().getLineTop(getLineCount()) +
        ...
        // 获取收缩时 的文字的高度
        mCollapsedTextHeight = getLayout().getLineTop(mMaxCollapsedLines) +
                               getCompoundPaddingTop() + getCompoundPaddingBottom();
