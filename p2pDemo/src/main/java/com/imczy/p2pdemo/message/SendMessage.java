...
public final class SendMessage extends Message
{
    ...
    public boolean checkFormat()
    {
        ...
        LogUtil.d(TAG, "data.length != dataLen + CLIENT_MESSAGE_MIN_LENGTH = " +
                  (data.length != dataLen + CLIENT_MESSAGE_MIN_LENGTH));
    ...
    public static SendMessage newMessage(int appId, int msgType, byte[] uuid,
                                         String contentStr)
    {
        if (contentStr == null) {
            contentStr = "";
        ...
        byte[] data = new byte[SendMessage.CLIENT_MESSAGE_MIN_LENGTH +
                               contentBytes.length];
