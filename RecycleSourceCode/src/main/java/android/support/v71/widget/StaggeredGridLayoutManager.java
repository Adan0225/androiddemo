...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager
{
    ...
    private boolean checkForGaps()
    {
        ...
        int invalidGapDir = mShouldReverseLayout ? LayoutState.LAYOUT_START :
    ...
    private void ensureOrientationHelper()
    {
        if (mPrimaryOrientation == null) {
            mPrimaryOrientation = OrientationHelper.createOrientationHelper(this,
                                  mOrientation);
    ...
    boolean updateAnchorFromPendingData(RecyclerView.State state,
                                        AnchorInfo anchorInfo)
    {
    ...
    void updateMeasureSpecs()
    {
        ...
        if (mOrientation == VERTICAL) {
            mWidthSpec = View.MeasureSpec.makeMeasureSpec(mSizePerSpan,
                         View.MeasureSpec.EXACTLY);
            mHeightSpec = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
        } else {
            mHeightSpec = View.MeasureSpec.makeMeasureSpec(mSizePerSpan,
                          View.MeasureSpec.EXACTLY);
    ...
    @Override
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler
            recycler,
    ...
    private void fixStartGap(RecyclerView.Recycler recycler,
                             ...
                             boolean canOffsetChildren)
    {
        final int minStartLine = getMinStart(
                                     mPrimaryOrientation.getStartAfterPadding());
    ...
    private void setLayoutStateDirection(int direction)
    {
        ...
        mLayoutState.mItemDirection = (mShouldReverseLayout == (direction ==
     ...
     */
    private void handleUpdate(int positionStart, int itemCountOrToPosition,
                              int cmd)
    {
        int minPosition = mShouldReverseLayout ? getLastChildPosition() :
                          getFirstChildPosition();
        ...
        int maxPosition = mShouldReverseLayout ? getFirstChildPosition() :
                     ...
                     RecyclerView.State state)
    {
        ...
        while (layoutState.hasMore(state) && !mRemainingSpans.isEmpty()) {
            ...
            final int otherStart = lp.mFullSpan ?
                                   mSecondaryOrientation.getStartAfterPadding()
                                   : currentSpan.mIndex * mSizePerSpan +
                                   mSecondaryOrientation.getStartAfterPadding();
            final int otherEnd = otherStart + mSecondaryOrientation.getDecoratedMeasurement(
                                     view);
    ...
    private LazySpanLookup.FullSpanItem createFullSpanItemFromStart(
        int newItemBottom)
    {
        LazySpanLookup.FullSpanItem fsi = new LazySpanLookup.FullSpanItem();
        fsi.mGapPerSpan = new int[mSpanCount];
    ...
    private void attachViewToSpans(View view, LayoutParams lp,
    ...
    private void layoutDecoratedWithMargins(View child, int left, int top,
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView,
                                       ...
                                       int position)
    {
        android.support.v71.widget.LinearSmoothScroller scroller = new
        android.support.v71.widget.LinearSmoothScroller(recyclerView.getContext()) {
            @Override
    ...
    {
        if (mPendingSavedState != null &&
            mPendingSavedState.mAnchorPosition != position) {
    ...
    int scrollBy(int dt, RecyclerView.Recycler recycler, RecyclerView.State state)
    {
        ...
        mLayoutState.mCurrentPosition = referenceChildPosition +
                                        mLayoutState.mItemDirection;
    ...
    @Override
    public RecyclerView.LayoutParams generateLayoutParams(ViewGroup.LayoutParams
            lp)
    {
    ...
    static class LazySpanLookup
    {
         ...
         */
        public FullSpanItem getFirstFullSpanItemInRange(int minPos, int maxPos,
                int gapDir,
