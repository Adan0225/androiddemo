...
public class AutoHeightScrollView extends ScrollView
{
    ...
    {
        int defaultHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                            DEFAULT_MAX_HEIGHT_DP, getResources().getDisplayMetrics());

        if (attrs != null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs,
                                    R.styleable.AutoHeightScrollView);
            mMaxHeight = typedArray.getDimensionPixelSize(
                             R.styleable.AutoHeightScrollView_maxHeight, defaultHeight);
            typedArray.recycle();
        } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        if (getChildCount() > 0) {
            ...
            Log.e(TAG, "onMeasure: child.getMeasuredHeight() = " +
