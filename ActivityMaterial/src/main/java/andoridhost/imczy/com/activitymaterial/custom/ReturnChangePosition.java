...
public class ReturnChangePosition extends Transition
{
    ...
    private static final String PROPNAME_POSITION =
        "custom_position:change_position:position";
    ...
    public ReturnChangePosition()
    {
        setPathMotion(new PathMotion() {
            ...
            public Path getPath(float startX, float startY, float endX, float endY) {
                ...
                Log.d(TAG, "getPath() called with: " + "startX = [" + startX + "], startY = [" +
                                   ...
                                   TransitionValues startValues, TransitionValues endValues)
    {
        ...
        if (startValues.view.getId() > 0) {
            ...
            Log.e(TAG, "createAnimator: startRect = " + startRect + " , endRect = " +
                  endRect);

            Path changePosPath = getPathMotion().getPath(startRect.centerX(),
                                 startRect.centerY(), endRect.centerX(),
                                 endRect.centerY() - endRect.height() / 2);

            int radius = startRect.centerY() - endRect.centerY();

            Log.e(TAG, "createAnimator: startRect center x = " + startRect.centerX() +
                  " , centerY= " + startRect.centerY());
            Log.w(TAG, "createAnimator: end rect  center x = " + endRect.centerX() +
                  " , centerY= " + endRect.centerY());

            ObjectAnimator objectAnimator = ObjectAnimator.ofObject(view,
                                            new PropPosition(PointF.class, "position", new PointF(startRect.centerX(),
                                                    startRect.centerY())), null, changePosPath);
