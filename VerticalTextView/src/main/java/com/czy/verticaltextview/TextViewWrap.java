...
public class TextViewWrap extends LinearLayout
{
    ...
    public void hidden2()
    {
        ...
        for (int i = mTextViewList.size() - 1; i >= 0; i--) {
            ...
            ObjectAnimator rotationYAnimator = ObjectAnimator.ofFloat(mTextViewList.get(i),
                                               "rotationY", mTextViewList.get(i).getRotationY(), 90);
            ObjectAnimator alphaAnia = ObjectAnimator.ofFloat(mTextViewList.get(i), "alpha",
                                       mTextViewList.get(i).getAlpha(), 0);
        ...
        for (int i = 0; i < animatorArrayList.size() - 1; i++) {
            ...
            ObjectAnimator childAnima = (ObjectAnimator) animator.getChildAnimations().get(
                                            0);
    ...
    public void hidden()
    {
        ...
        for (int i = mTextViewList.size() - 1; i >= 0; i--) {
            ...
            mTextViewList.get(i).setPivotY(mTextViewList.get(i).getHeight() / 2);
            AnimatorSet anim = new AnimatorSet();
            ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(mTextViewList.get(i),
                                            "rotationY", mTextViewList.get(i).getRotationY(), 90);
            ObjectAnimator alphaAnia = ObjectAnimator.ofFloat(mTextViewList.get(i), "alpha",
                                       mTextViewList.get(i).getAlpha(), 0);
    ...
    public void show()
    {
        ...
        for (int i = 0, z = mTextViewList.size(); i < z; i++) {
            ...
            ObjectAnimator alphaAnia = ObjectAnimator.ofFloat(mTextViewList.get(i), "alpha",
                                       mTextViewList.get(i).getAlpha(), 1);
            anim.playTogether(objectAnimator, alphaAnia);
            objectAnimators.add(anim);
        }
