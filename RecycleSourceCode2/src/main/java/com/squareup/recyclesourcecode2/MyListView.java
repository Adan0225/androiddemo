...
public class MyListView extends ListView implements
    AbsListView.OnScrollListener, View.OnTouchListener
{
                         ...
                         int visibleItemCount, int totalItemCount)
    {
        ...
        if (mScrollState == SCROLL_STATE_TOUCH_SCROLL) {
            ...
            Log.d("test", "direction:" + (mScrollDirection == SCROLLING_UP ? "up" : "down")
                  + ", scrollOffset:" + mScrollOffset + ", toucheId:" + mTouchedIndex +
                  ", fvisible:" + firstVisibleItem + ", " +
            ...
            if (mScrollDirection == SCROLLING_DOWN) {
                setAnimationForScrollingDown(mTouchedIndex - getPositionForView(getChildAt(0)),
                                             indexOfLastAnimatedItem, firstVisibleItem);
            } else {
                setAnimationForScrollingUp(mTouchedIndex - getPositionForView(getChildAt(0)),
                                           indexOfLastAnimatedItem, firstVisibleItem);
            }
    ...
    private void setAnimationForScrollingDown(int indexOfTouchedChild,
            int indexOflastAnimatedChild, int firstVisibleIndex)
    {
        for (int i = indexOfTouchedChild + 1; i <= indexOflastAnimatedChild; i++) {
            ...
            if (!animatedItems.containsKey(v)) {
                animatedItems.put(v, v.animate().translationY(0).setDuration(300).setStartDelay(
                                      50 * i));
            }
    ...
    private void setAnimationForScrollingUp(int indexOfTouchedChild,
                                            int indexOflastAnimatedChild, int firstVisibleIndex)
    {
