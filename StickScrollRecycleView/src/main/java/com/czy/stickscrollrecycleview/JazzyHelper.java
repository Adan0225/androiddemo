...
public class JazzyHelper
{
    ...
    public final void onScrolled(ViewGroup viewGroup, int touchItem,
                                 int firstVisibleItem, int visibleItemCount, int totalItemCount, int dy)
    {
        touchPos = touchItem;
        // 是否应该动画 判断参数
        boolean shouldAnimateItems = (mFirstVisibleItem != -1 &&
                                      mLastVisibleItem != -1);
        ...
        if (mIsScrolling && shouldAnimateItems) {
            ...
            while (lastVisibleItem - indexBeforeLast > touchItem) {
                View item = viewGroup.getChildAt(lastVisibleItem - firstVisibleItem -
                                                 indexBeforeLast);
