...
public class VerticalStepView extends ViewGroup
{
    ...
    {
        mCompleteDrawable = context.getResources().getDrawable(
                                R.drawable.icon_point_light);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        int contentWidth = widthSize - getPaddingLeft() - getPaddingRight() -
                           drawableWidth - mDrawablePaddingItemVal;

        int childMeasureWidthSpec = MeasureSpec.makeMeasureSpec(contentWidth,
                                    MeasureSpec.AT_MOST);
        int childMeasureHeightSpec = MeasureSpec.makeMeasureSpec(0,
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i != getChildCount() - 1) {
                allItemHeight = allItemHeight + getChildAt(i).getMeasuredHeight() +
                                mItemDieviderHeight;
            } else {
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (getChildCount() > 0) {
            lineBottom = getHeight() - getChildAt(getChildCount() - 1).getMeasuredHeight() -
                         getPaddingBottom();
        }
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i == 0) {
                mDoingDrawable.setBounds(drawableLeft, drawableTop, drawableRight,
                                         drawableBottom);
                mDoingDrawable.draw(canvas);
            } else {
                mCompleteDrawable.setBounds(drawableLeft, drawableTop, drawableRight,
                                            drawableBottom);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b)
    {
        ...
        int layoutLeft = l + getPaddingLeft() + mDoingDrawable.getIntrinsicWidth() +
                         mDrawablePaddingItemVal;
