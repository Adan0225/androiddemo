...
public class UDPClient implements Runnable
{
    ...
    protected ConcurrentLinkedQueue<ReceiverMessage> mMessageQueue = new
    ConcurrentLinkedQueue<ReceiverMessage>();
    ...
    public static UDPClient getInstance(Context context,
        ...
        if (udpClient == null)
        {
            ...
            LogUtil.d(TAG, "mUuid length = " + uuidBytes.length + " , mUuid = " +
                      uuid.toString());
            udpClient = new UDPClient(uuidBytes, 1, Constant.DANMAKU_SERVER_HOSTNAME,
                                      Constant.DANMAKU_SERVER_PORT, context);
        }
        ...
        while (!mStoped) {
            try {
                ...
                if (!mIsLogin) {
                    login();
                    Thread.sleep(
                        30);//防止login和heartbeat两次请求间隔太短，导致heartbeat命令包丢失
                }
    ...
    private void heartbeat()
    {
        ...
        try {
            ...
            SendMessage message = SendMessage.newMessage(mAppid, SendMessage.CMD_HEARTBEAT,
                                  mUuid, "");
    ...
    private void login()
    {
        ...
        String gps = "0,0";
        String group = "A:ALL";
        String ipPort = "" + NetworkUtil.getLocalIpAddress(mContext) + ":" +
    ...
    private void login(int userId, String group, String gps)
    {
        try {
            ...
            SendMessage loginMessage = SendMessage.newMessage(mAppid, SendMessage.CMD_LOGIN,
                                       mUuid, userId + "|" + group + "|" + gps + "|" + NetworkUtil.getLocalIpAddress(
                                           mContext) + ":" + mClient.getLocalPort());
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mIsLogin = false;
                    SendMessage message = SendMessage.newMessage(mAppid, SendMessage.CMD_LOGOUT,
                                          mUuid, "");
                    send(message.getData());
                    stop();
                    mP2PClient.logout();
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SendMessage message = SendMessage.newMessage(mAppid, SendMessage.CMD_CHAT,
                                          mUuid, msg);
                    send(message.getData());
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SendMessage message = SendMessage.newMessage(mAppid, SendMessage.CMD_CHAT_P2P,
                                          mUuid, uuid);
                    send(message.getData());
                } catch (Exception e) {
                    e.printStackTrace();
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SendMessage message = SendMessage.newMessage(mAppid,
                                          SendMessage.CMD_I_HAD_CALL_A, mUuid, uuid);
                    send(message.getData());
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SendMessage message = SendMessage.newMessage(mAppid, SendMessage.CMD_POSITION,
                                          mUuid, postion);
                    send(message.getData());
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    class Worker implements Runnable
    {
        ...
        private synchronized void handleEvent() throws Exception {
            ...
            while (true)
            {
                ...
                //real work here
                receiverMessageNum++;
                LogUtil.e(TAG, "onPushMessage receiverMessageNum = " + receiverMessageNum +
    ...
    public void setOnReceiverMessageListener(OnReceiverMessageListener
            onReceiverMessageListener)
    {
