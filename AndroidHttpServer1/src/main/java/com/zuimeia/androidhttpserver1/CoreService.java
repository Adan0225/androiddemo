...
public class CoreService extends Service
{
    ...
    private void exeStartHttpServer()
    {
        ...
        mHttpServer.addHandler("*",
                               smartHandler);// 配置路由分发，必须加smartHandler；用于自动匹配
        //根据业务去配置请求路径
        mHttpServer.addHandler("/index",
                               new ResultHandler(this));//比如请求 http://127.0.0.1:12345/index 就会在resultHandler上处理；


        mHttpServer.start();
        mHttpServer.setStartCompleteListener(new
        TuziHttpServer.StartCompleteListener() {
