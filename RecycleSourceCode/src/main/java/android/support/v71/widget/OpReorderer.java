...
class OpReorderer
{
     ...
     * @param next
     */
    private void swapMoveOp(List<AdapterHelper.UpdateOp> list, int badMove,
                            int next)
    {
        final AdapterHelper.UpdateOp moveOp = list.get(
                badMove);// 上一次的move 操作
     ...
     */
    void swapMoveRemove(List<AdapterHelper.UpdateOp> list, int movePos,
                        ...
                        int removePos, AdapterHelper.UpdateOp removeOp)
    {
        ...
        if (moveOp.positionStart <= removeOp.positionStart) {
            ...
            extraRm = mCallback.obtainUpdateOp(AdapterHelper.UpdateOp.REMOVE,
                                               moveOp.positionStart + 1, remaining, null);
    ...
    private void swapMoveAdd(List<AdapterHelper.UpdateOp> list, int move,
                             AdapterHelper.UpdateOp moveOp, int add,
    ...
    void swapMoveUpdate(List<AdapterHelper.UpdateOp> list, int move,
                        AdapterHelper.UpdateOp moveOp, int update,
                        AdapterHelper.UpdateOp updateOp)
    {
        ...
        if (moveOp.itemCount < updateOp.positionStart) {
            ...
            updateOp.itemCount--;
            extraUp1 = mCallback.obtainUpdateOp(AdapterHelper.UpdateOp.UPDATE,
        ...
        if (moveOp.positionStart <= updateOp.positionStart) {
            ...
            final int remaining = updateOp.positionStart + updateOp.itemCount
                                  - moveOp.positionStart;
            extraUp2 = mCallback.obtainUpdateOp(AdapterHelper.UpdateOp.UPDATE,
                                                moveOp.positionStart + 1, remaining,
                                                updateOp.payload);
