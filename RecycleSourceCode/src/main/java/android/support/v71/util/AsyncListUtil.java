...
public class AsyncListUtil<T>
{
    ...
    final android.support.v71.util.ThreadUtil.MainThreadCallback<T>
    mMainThreadProxy;
    final android.support.v71.util.ThreadUtil.BackgroundCallback<T>
    mBackgroundProxy;
    ...
    public T getItem(int position)
    {
        if (position < 0 || position >= mItemCount) {
            throw new IndexOutOfBoundsException(position + " is not within 0 and " +
                                                mItemCount);
        }
    ...
    private void updateRange()
    {
        ...
        mTmpRangeExtended[0] = Math.min(mTmpRange[0], Math.max(mTmpRangeExtended[0],
                                        0));
        mTmpRangeExtended[1] =
    ...
    mMainThreadCallback = new
    android.support.v71.util.ThreadUtil.MainThreadCallback<T>()
    {
        @Override
        ...
        public void removeTile(int generation, int position) {
            ...
            if (DEBUG) {
                log("recycling tile @%d, total tiles: %d", tile.mStartPosition,
                    mTileList.size());
            }
    ...
    mBackgroundCallback = new
    android.support.v71.util.ThreadUtil.BackgroundCallback<T>()
    {
        ...
        public void updateRange(int rangeStart, int rangeEnd, int extRangeStart,
            ...
            if (scrollHint == ViewCallback.HINT_SCROLL_DESC) {
                ...
                requestTiles(lastVisibleTileStart + mTileSize, mLastRequiredTileStart,
                             scrollHint,
                             false);
            } else {
                requestTiles(firstVisibleTileStart, mLastRequiredTileStart, scrollHint, false);
                requestTiles(mFirstRequiredTileStart, firstVisibleTileStart - mTileSize,
                             scrollHint,
                             true);
            }
        }
        ...
        private void addTile(TileList.Tile<T> tile) {
            ...
            if (DEBUG) {
                log("loaded tile @%d, total tiles: %d", tile.mStartPosition,
                    mLoadedTiles.size());
            }
        }
    ...
    public static abstract class ViewCallback
    {
        ...
        public void extendRangeInto(int[] range, int[] outRange, int scrollHint)
        {
            ...
            outRange[0] = range[0] - (scrollHint == HINT_SCROLL_DESC ? fullRange :
                                      halfRange);
            outRange[1] = range[1] + (scrollHint == HINT_SCROLL_ASC ? fullRange :
                                      halfRange);
        }
