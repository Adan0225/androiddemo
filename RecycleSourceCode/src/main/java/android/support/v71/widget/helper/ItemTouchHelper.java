    ...
    implements RecyclerView.OnChildAttachStateChangeListener
{
    ...
    private static final int ACTION_MODE_IDLE_MASK = (1 << DIRECTION_FLAG_COUNT) -
            1;
        ...
        = new OnItemTouchListener()
    {
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView,
            ...
            if (DEBUG) {
                Log.d(TAG, "intercept: x:" + event.getX() + ",y:" + event.getY() + ", " +
                      event);
            }
    ...
    private static boolean hitTest(View child, float x, float y, float left,
                                   float top)
    {
        return x >= left &&
    ...
    private ViewHolder findSwipedView(MotionEvent motionEvent)
    {
        ...
        final int pointerIndex = MotionEventCompat.findPointerIndex(motionEvent,
                                 mActivePointerId);
        final float dx = MotionEventCompat.getX(motionEvent,
                                                pointerIndex) - mInitialTouchX;
        final float dy = MotionEventCompat.getY(motionEvent,
                                                pointerIndex) - mInitialTouchY;
    ...
    public abstract static class Callback
    {
        ...
        private static final android.support.v71.widget.helper.ItemTouchUIUtil
        sUICallback;
        ...
        private static final Interpolator sDragViewScrollCapInterpolator = new
        Interpolator()
        {
            public float getInterpolation(float t) {
                t -= 1.0f;
        ...
        static {
            ...
            {
                sUICallback = new
                android.support.v71.widget.helper.ItemTouchUIUtilImpl.Lollipop();
            } else if (Build.VERSION.SDK_INT >= 11)
            {
                sUICallback = new
                android.support.v71.widget.helper.ItemTouchUIUtilImpl.Honeycomb();
            } else
            {
                sUICallback = new
                android.support.v71.widget.helper.ItemTouchUIUtilImpl.Gingerbread();
            }
        }
         ...
         */
        public static android.support.v71.widget.helper.ItemTouchUIUtil
        getDefaultUIUtil()
        {
            return sUICallback;
        }
                                           ...
                                           ViewHolder viewHolder)
        {
            ...
            return convertToAbsoluteDirection(flags,
        ...
        public void onMoved(final RecyclerView recyclerView,
                            final ViewHolder viewHolder, int fromPos, final ViewHolder target, int toPos,
                            int x,
                            int y)
        {
                                ...
                                int actionState, float dX, float dY)
        {
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                final ItemTouchHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                final int count = c.save();
                onChildDrawOver(c, parent, anim.mViewHolder, anim.mX, anim.mY,
                                anim.mActionState,
                                false);
                c.restoreToCount(count);
            }
        ...
        public void onChildDrawOver(Canvas c, RecyclerView recyclerView,
                                    ...
                                    float dX, float dY, int actionState, boolean isCurrentlyActive)
        {
            sUICallback.onDrawOver(c, recyclerView, viewHolder.itemView, dX, dY,
                                   actionState,
                                   isCurrentlyActive);
        }
